...
class S3Uploader(BaseS3Uploader):
    ...
    def update_data_dict(self, data_dict, url_field, file_field, clear_field):
        ...
        if hasattr(self.upload_field_storage, 'filename'):
            ...
            self.remote_filepath = os.path.join('https://s3.amazonaws.com/',self.bucket_name, self.filepath)
    ...
    def upload(self, max_size=2):
        ...
        if self.filename:
            self.upload_to_key(self.filepath, self.upload_file, make_public=True)
            self.clear = True

        if (self.clear and self.old_filename and not self.old_filename.startswith('http')):
